...
# Validate the Terraform doc generator installation and version
function validate_terraform_docs(){

    if [ ! "$(command -v terraform-docs )" ]; then
        echo_missing_installation "Terraform-docs" "https://github.com/terraform-docs/terraform-docs"
        ERRORS+=$'  Terraform-docs not found\n'
    fi
}
...
function validate_git(){
    ...
    if ! git config --global init.defaultBranch | grep "main" >/dev/null ; then
