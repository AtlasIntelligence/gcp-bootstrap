...
resource "google_dns_record_set" "record_set" {
  # use subdomain if provided. Else use dns name.
  name = var.service_dns_prefix != "" ? "${lower(var.service_dns_prefix)}.${data.google_dns_managed_zone.dns_zone.dns_name}" : data.google_dns_managed_zone.dns_zone.dns_name
  type = "A"
  ttl  = 300
